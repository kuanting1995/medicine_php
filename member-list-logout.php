...
if (isset($_GET['state_sid'])) {
  ...
  if ($total > 0) {
    ...
    $sql = sprintf("SELECT * FROM `members` ORDER BY `member_id` $myorder LIMIT %s, %s", ($page - 1) * $per_page, $per_page);
    $rows = $pdo->query($sql)->fetchAll();
  }
}
...
<div class="container w-75">
    ...
    <button class="btn btn-primary mt-3 d-block" style="margin-left:auto;"
        onclick="location.href ='member-add.php'">新增會員</button>
    <div class="d-flex justify-content-center gap-5 align-items-center">
        <!-- 搜尋功能   -->
        <form name="form1" class="d-flex mt-3" onsubmit="event.preventDefault()">
            <input class="form-control me-2 " type="search" placeholder="請輸入姓名" aria-label="Search" id="search"
                name="search">
            <button class="btn btn-outline-secondary" type="submit" onclick="nameSearch()"><i
                    class="fa-solid fa-magnifying-glass"></i></button>
        </form>
        <!-- 篩選功能 -->
        <div class="dropdown">
            <button class="btn btn-outline-secondary dropdown-toggle mt-3" type="button" id="dropdownMenuButton1"
            ...
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                ...
                <li><a class="dropdown-item text-center"
                        href="member-list.php?state_sid=1"><?= $state[0]['member_state'] ?></a></li>
                <li><a class="dropdown-item text-center"
                        href="member-list.php?state_sid=2"><?= $state[1]['member_state'] ?></a></li>
                <li><a class="dropdown-item text-center"
                        href="member-list.php?state_sid=3"><?= $state[2]['member_state'] ?></a></li>
            </ul>
        </div>
    </div>
    ...
    <nav aria-label="Page navigation example" class="d-flex justify-content-center mt-3">
        <ul class="pagination">
            ...
            <li class="page-item <?= $page == 1 ? 'disabled' : '' ?>">
                <a class="page-link" href="?orderby=1&<?= 'state_sid=' . $state_sid . '&page=' . $page - 1 ?>">
            ...
            <li class="page-item <?= $page == $i ? 'active' : '' ?> ">
                <a class="page-link" href="?orderby=1&<?= 'state_sid=' . $state_sid . '&page=' . $i ?>"><?= $i ?></a>
            </li>
            <?php }; ?>
            <?php }; ?>
            <!-- 到下一頁，最後一頁按鈕不能按 -->
    ...
    <nav aria-label="Page navigation example" class="d-flex justify-content-center mt-3">
        <ul class="pagination">
            <!-- 回上一頁，第一頁按鈕不能按 -->
            <li class="page-item <?= $page == 1 ? 'disabled' : '' ?>">
                <a class="page-link" href="?orderby=1&<?= 'search=' . $search . '&page=' . $page - 1 ?>">
                    <i class="fa-solid fa-circle-arrow-left"></i>
                </a>
            </li>
            <!-- 跑迴圈帶出頁碼，當前頁數往前5頁+往後5頁 -->
            <?php for ($i = $page - 5; $i <= $page + 5; $i++) {
              ...
              if ($i >= 1 and $i <= $total_page) {  ?>
            <li class="page-item <?= $page == $i ? 'active' : '' ?> ">
                <a class="page-link" href="?orderby=1&<?= 'search=' . $search . '&page=' . $i ?>"><?= $i ?></a>
            </li>
            <?php }; ?>
            <?php }; ?>
            <!-- 到下一頁，最後一頁按鈕不能按 -->
    ...
    <nav aria-label="Page navigation example" class="d-flex justify-content-center mt-3">
        <ul class="pagination">
            <!-- 回上一頁，第一頁按鈕不能按 -->
            <li class="page-item <?= $page == 1 ? 'disabled' : '' ?>">
                <a class="page-link" href="?orderby=1&page=<?= $page - 1 ?>">
                    <i class="fa-solid fa-circle-arrow-left"></i>
                </a>
            </li>
            <!-- 跑迴圈帶出頁碼，當前頁數往前5頁+往後5頁 -->
            <?php for ($i = $page - 5; $i <= $page + 5; $i++) {
              ...
              if ($i >= 1 and $i <= $total_page) {  ?>
            <li class="page-item <?= $page == $i ? 'active' : '' ?> ">
                <a class="page-link" href="?orderby=1&page=<?= $i ?>"><?= $i ?></a>
            </li>
            <?php }; ?>
            <?php }; ?>
            <!-- 到下一頁，最後一頁按鈕不能按 -->
    ...
    <table class="table table-striped">
        <thead>
            <tr>
                <th scope="col">編號<i class="fa-solid fa-caret-down ms-2" onclick="location.href ='<?php if (isset($_GET['state_sid'])) {
                                                                                              echo '?orderby=1&state_sid=' . $_GET['state_sid'] . '&page=' . $page;
                                                                                            } else if (isset($_GET['search'])) {
                                                                                              echo '?orderby=1&search=' . $_GET['search'] . '&page=' . $page;
                                                                                            } else {
                                                                                              echo '?orderby=1';
                                                                                            } ?>'"></i></th>
                <th scope="col">信箱</th>
